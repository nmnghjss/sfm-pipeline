...
def load_slam_data(slam_dir):
    ...
    cameras_txt = os.path.join(slam_dir, "cameras.txt")
    ...
    num_keyframes, num_2d, num_3d = map(
        int,
        lines[0].split()
    )
    ...
    for line in lines[2:]:
        ...
        rvec = np.array(vals[:3], dtype=np.float64)
        twc = np.array(vals[3:], dtype=np.float64)
...
if __name__ == "__main__":
    ...
    slam_dir = "E:\\SLAM_Data\\heyu20240718-4"

    output_dir = "E:\\SLAM_Data\\heyu20240718-4\\colmap_model"
