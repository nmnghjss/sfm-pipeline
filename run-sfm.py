...
if feature_match_type != "UNDEFINED":
    ...
    elif args.feature_type == "LOMA":
        feature_match_type = "LOMA"
    else:
...
if args.feature_type == "LOMA":
        ...
        extract_time, feature_matching_time = match_features_with_loma(
            ...
            match_strategy=args.match_strategy,              # ★ 透传匹配策略
            max_matches_per_image=args.max_matches_per_image,
            min_matches_per_image=args.min_matches_per_image,
            similarity_threshold=args.similarity_threshold,
            num_keypoints=args.max_feature_num,
            descriptor_type_id=3,                            # ★ LOMA 在 db 中的 type id
            logger=logger
        )
        feature_extraction_time = extract_time
        image_features = ("LOMA_DONE",)  # 占位，让下面的匹配分支跳过

        logger.info("LOMA matching already written to db, running matches_importer for geometric verification...")
        matches_importer_cmd = get_matches_importer_cmd(
            ...
            feature_match_type="UNDEFINED",   # ★ LOMA 已写入 raw matches，类型用 UNDEFINED 即可
            use_gpu=use_gpu,
        ...
        logger.info(f"start to run subprocess for matches_importer")
        run_subprocess(matches_importer_cmd, logger)
        feature_matching_time = feature_matching_time  # 已在前面记录
        logger.info(f"Geometric verification done, total feature+matches time: {feature_matching_time:.2f} s")
else:
