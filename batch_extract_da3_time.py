...
# from train import *
from utils import count_image_files,  delete_directory, delete_file, find_directories_by_name, move_files
...
def extract_da3_time_from_log(sfm_output_dir):
    """
    从 sfm_output_dir 中唯一的 .log 文件中提取 DA3 耗时和图像数量。

    解析 log 中:
      "input images num: XX"          → 图像数量 + 起始时间戳
      "get pose with DA3 successful"   → 结束时间戳

    参数:
        sfm_output_dir: SfM 输出目录路径

    返回:
        (image_count, da3_elapsed): 图像数量 (int) 和 DA3 耗时 (float, 秒)
        如果找不到 log 文件或关键行，返回 (0, 0.0)
    """
    log_files = glob.glob(os.path.join(sfm_output_dir, "*.log"))
    if len(log_files) != 1:
        print(f"[WARN] Expected 1 .log file in {sfm_output_dir}, found {len(log_files)}")
        return 0, 0.0
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for line in f:
            # 匹配 "input images num: XX"
            m = re.search(r"input images num:\s*(\d+)", line)
            if m:
                image_count = int(m.group(1))
                ts_start = datetime.strptime(line[:23], ts_format)
                print(f"[INFO] Found: input images num={image_count}, ts={ts_start}")

            # 匹配 "get pose with DA3 successful"
            if "get pose with DA3 successful" in line:
                ts_end = datetime.strptime(line[:23], ts_format)
                print(f"[INFO] Found: get pose with DA3 successful, ts={ts_end}")

    if ts_start is None or ts_end is None:
        print("[WARN] Could not find required log lines in log file")
        return image_count, 0.0

    da3_elapsed = (ts_end - ts_start).total_seconds()
    print(f"[INFO] DA3 elapsed: {da3_elapsed:.2f}s for {image_count} images")

    return image_count, da3_elapsed
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_name", "-o", type=str, default="output-sfm-aliked-threshold-with-DA3-incremental", help='输出目录名称，位于每个数据集目录下')
    parser.add_argument("--metrics_file_name", "-mf", type=str, default="metric_sfm-aliked-threshold-with-DA3-incremental.csv", help='保存所有数据集时间和指标的csv文件名')
    ...
    for dataset_dir in dataset_path:
        ...
        if "sfm" in args.pipeline:
            ...
            sfm_command = "python sfm-colmap-v4-with-DA3.py --clean"
            ...
            # 从 log 中提取 DA3 耗时
            da3_img_count, da3_elapsed = extract_da3_time_from_log(sfm_output_dir)
            time_and_metrics.da3_time = da3_elapsed
            if da3_img_count > 0 and da3_elapsed > 0:
                da3_data_points.append((da3_img_count, da3_elapsed))
    ...
    if da3_data_points:
        # 按图像数量从小到大排序
        da3_data_points.sort(key=lambda x: x[0])
        img_counts = [p[0] for p in da3_data_points]
        elapsed_times = [p[1] for p in da3_data_points]

        plt.figure(figsize=(10, 6))
        plt.plot(img_counts, elapsed_times, marker="o", linestyle="-", color="steelblue")
        plt.xlabel("Image Count")
        plt.ylabel("DA3 Elapsed Time (s)")
        plt.title("DA3 Pose Estimation Time vs Image Count")
        plt.grid(True, alpha=0.3)

        # 标注每个数据点
        for x, y in zip(img_counts, elapsed_times):
            plt.annotate(f"{y:.1f}s", (x, y), textcoords="offset points",
                         xytext=(0, 10), ha="center", fontsize=8)

        plot_path = os.path.join(args.dataset_root, "da3_time_vs_image_count.png")
        plt.tight_layout()
        plt.savefig(plot_path, dpi=150)
        plt.close()
        print(f"[INFO] DA3 time plot saved to: {plot_path}")
    else:
        print("[WARN] No DA3 data points collected, skipping plot.")