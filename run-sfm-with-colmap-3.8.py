...
def run_subprocess(cmd: list, log_path: str, hide_cmd: bool = True, log_output: bool = True):
    """
    Run a subprocess command, optionally print stdout/stderr in real-time and save to log file.
    Windows compatible.

    Parameters:
    - cmd: list - command to run
    - log_path: str - path to write a per-command log (used only when `log_output` is True)
    - hide_cmd: bool - if True, do not log the command string with the global logger
    - log_output: bool - if False, do not print subprocess output to the terminal or write to `log_path`.
    """
    if not hide_cmd:
    ...
    # If requested, open the per-command log and write output; otherwise just drain stdout.
    if log_output:
        ...
        # Drain stdout without printing or writing to file to avoid blocking.
...
colmap_path = os.path.join(current_path, "colmap-x64-windows-cuda-3.8/COLMAP.bat")
...
if Path(output_path).is_absolute() is False:
...
if args.acc:
    feat_matching_cmd = [
        ...
        "--SiftMatching.guided_matching", "0", # 0
        "--SiftMatching.max_ratio", "0.8", # 0.8
        "--SiftMatching.max_distance", "0.7", # 0.7
        "--SiftMatching.max_error", "4", # 4
        "--SiftMatching.confidence", "0.9999", # 0.999
        "--SiftMatching.min_inlier_ratio", "0.5", # 0.25
        "--SiftMatching.min_num_inliers", "25", # 15        
        "--SiftMatching.compute_relative_pose", "0", # 0
...
if args.acc:
    mapper_cmd = [
        ...
        "--Mapper.ba_global_images_ratio", "2.0", # 1.1
        "--Mapper.ba_global_points_ratio", "2.0", # 1.1
        "--Mapper.ba_global_images_freq", "5000", # 5000
