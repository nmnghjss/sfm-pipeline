...
from utils_colmap import estimate_depth_range 
# --------------------------
# Argument parser
# --------------------------
...
# aligned_dir = os.path.join(output_path, "sparse_aligned")
# os.makedirs(aligned_dir, exist_ok=True)
# os.makedirs(os.path.join(aligned_dir, "0"), exist_ok=True)
# start_orient = time.time() 
# orient_cmd = [
#     colmap_command, "model_orientation_aligner",
#     "--input_path", os.path.join(output_path, "sparse/0"),
#     "--image_path", os.path.join(output_path, "images"),
#     "--output_path", os.path.join(output_path, "sparse_aligned/0"),
# ]
# run_subprocess(orient_cmd, logger)
# orient_time = time.time() - start_orient
# logger.info(f"Model orientation aligner done, used time: {orient_time:.2f} s.")
...
dmin, dmax = estimate_depth_range(os.path.join(output_path, "sparse"), pmin=1, pmax=99)
min = dmin * 0.8
max = dmax * 1.2
logger.info(f"Estimated depth range: min={dmin:.4f}, max={dmax:.4f}")

densify_cmd = [
    colmap_command, "patch_match_stereo",
    "--workspace_path", output_path,
    "--PatchMatchStereo.max_image_size", str(mvs_max_image_size), # -1
    "--PatchMatchStereo.gpu_index", "-1",
    "--PatchMatchStereo.depth_min", str(min), # -1
    "--PatchMatchStereo.depth_max", str(max), # -1
...
run_subprocess(densify_cmd, logger)
densify_time = time.time() - start_densify
logger.info(f"Point cloud densification done, used time: {densify_time:.2f} s.")

## =================== Depth fusion =================== ##
start_stereo_fusion = time.time()
stereo_fusion_cmd = [
    colmap_command, "stereo_fusion",
    "--workspace_path", output_path,
    "--output_path", os.path.join(output_path, "fused.ply"),
    # "--bbox_path", os.path.join(output_path, "bbox.txt"),
    # "--StereoFusion.mask_path", os.path.join(output_path, "mask.png"),
    "--StereoFusion.num_threads", "-1",
    "--StereoFusion.max_image_size", str(mvs_max_image_size), # -1
    ...
    "--StereoFusion.use_cache", "0"
]
run_subprocess(stereo_fusion_cmd, logger)
stereo_fusion_time = time.time() - start_stereo_fusion
logger.info(f"Depth fusion done, used time: {stereo_fusion_time:.2f} s.")
...
delaunay_cmd = [
    ...
    "--input_type", "dense",
    "--output_path", os.path.join(output_path, "delaunay.ply"),
...
run_subprocess(delaunay_cmd, logger)
delaunay_time = time.time() - start_delaunay
logger.info(f"Delaunay meshing done, used time: {delaunay_time:.2f} s.")
